...
- 변경 전 [`Rules.md`](./Rules.md)를 읽고 따른다. 역할별 확인 항목은 [`.agents/README.md`](./.agents/README.md)에서 선택하며 이 문서와 `Rules.md`를 대체하지 않는다.
- 사용자 보고·계획·이슈·PR·커밋·문서·코드 리뷰는 한국어로 작성한다. 코드 식별자·명령어·API와 라이브러리 고유 명칭은 필요한 범위에서 영문을 사용한다.
- App Router·TypeScript strict를 유지한다. 기본은 Server Component이며 `'use client'`는 상호작용하는 잎까지 내린다.
